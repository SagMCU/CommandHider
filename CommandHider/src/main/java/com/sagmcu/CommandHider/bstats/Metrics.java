 ...
 */
package com.sagmcu.CommandHider.bstats;
...
public class Metrics {
    ...
    public Metrics(Plugin plugin, int serviceId) {
        ...
        if (!config.isSet("serverUuid")) {
            ...
            // Save config to apply defaults
            try {
                config.save(configFile);
            } catch (IOException ignored) {
            }

            // Manually write the header after saving
            try {
                ...
                lines.addAll(0, header.stream().map(line -> "# " + line).collect(Collectors.toList())); // Add header
                                                                                                        // with '#'
